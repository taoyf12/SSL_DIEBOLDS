...
def read_list_edge(filename1, filename2):
    print 'reading list edges'    
    listedge = []
    for line in open(filename1):
        inputs = line.strip().split()
        listedge.append((inputs[0][:-8],inputs[1][:-8]))
    # print len(listedge)
...
def read_list_list_edge(filename1, filename2):
    print 'reading list-list edges'    
    lledge = dd(set)
    for line in open(filename1):
        inputs = line.strip().split()
        # print '8', inputs[0][:-8], inputs[1][:-8]
        lledge[inputs[0][:-8]].add(inputs[1][:-8])
        lledge[inputs[1][:-8]].add(inputs[0][:-8])
#     len1 = 0
#     for key in lledge.keys():
#         len1 += len(lledge[key])
#     print len(lledge.keys()), len1
    for line in open(filename2):
        inputs = line.strip().split()
        # print '8', inputs[0][:-8], inputs[1][:-8]
        lledge[inputs[0][:-8]].add(inputs[1][:-8])
...
def construct_graph(train_id, test_id, cites, listedge):
    id2index, cnt = {}, 0
    for id in train_id:
        # print id
...
def run(run_num, args, features, f_num, cites, s_graph, sim_dict, listedge):
    # yf: train_list-<set>: lists which contain the test items
    # in_labels-<dict>: catergory -> related lists
#     print 'start examination!'
#     for i, l in enumerate(lledge.keys()):
#         if i%10000 == 0:
#             print i, l
#         if l not in s_graph.keys():
#             print 'error!'
#             print l
#     print 'finished!'
...
#                 in_labels[cat].add(l)
    ...
    if not FEATURE_SAVED:
        ...
        print 'loading list features and labels'
    ...
    graph = construct_graph(train_list, test_list, cites, listedge)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--g_batch_size', help = 'the batch size for graph', type = int, default = 50) #zl: 50
    ...
    parser.add_argument('--g_learning_rate', help = 'learning rate for graph', type = float, default = 1e-2) #1e-1
    parser.add_argument('--train_step', type = int, default = 100) # 100
    args = parser.parse_args()

    # dir = '../diel'
    # lledge = read_list_list_edge('/remote/curtis/baidu/SSL-pipeline/main-exp/raw-data/u_dailymed200_RT/failRRPS_input.txt','/remote/curtis/baidu/SSL-pipeline/main-exp/raw-data/u_dailymed200_RT/failRRD_input.txt')
    listedge = read_list_edge('/remote/curtis/baidu/SSL-pipeline/main-exp/raw-data/u_dailymed200_RT/failRRPS_input.txt','/remote/curtis/baidu/SSL-pipeline/main-exp/raw-data/u_dailymed200_RT/failRRD_input.txt')
    if not FEATURE_SAVED:
    ...
    sim_dict = read_sim_dict('sim.dict')
    run(1, args, features, f_num, cites, s_graph, sim_dict, listedge)
    # for i in range(1, 10):
